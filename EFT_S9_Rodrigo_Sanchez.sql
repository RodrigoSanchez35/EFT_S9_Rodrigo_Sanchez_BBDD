...
------------------------------------------------------
-- Índice para evitar full scan en la tabla CLIENTE por región
CREATE INDEX IDX_REGION ON cliente (cod_region);

-- Índice para mejorar rendimiento mediante acceso por índice, 
-- se elige la columna fecha_inscripcion para permitir un mejor filtrado
CREATE INDEX IDX_CLI_REGION ON cliente (fecha_inscripcion);

------------------------------------------------------
-- Creación de la vista con el resumen solicitado
------------------------------------------------------
-- La vista muestra:
--  - Nombre de la región
--  - Cantidad de clientes con 20 o más años de inscripción
--  - Total de clientes por región
-- Ordenado por la cantidad de clientes con ≥20 años ascendente
CREATE OR REPLACE VIEW V_RESUMEN_CLIENTES_REGION AS
SELECT
    ...
    COUNT(CASE 
            ...
            THEN 1 
         END
    ) AS CLIENTES_20_Y_MAS,
    COUNT(*) AS TOTAL_CLIENTES_REGION
FROM
    cliente c
JOIN
...
------------------------------------------------------
-- Opcional: Mostrar plan de ejecución 
------------------------------------------------------
EXPLAIN PLAN FOR
SELECT * FROM V_RESUMEN_CLIENTES_REGION;
...
-- *************************************************************
-- * Informe 2: Transacciones con vencimientos en el 2do semestre *
-- *************************************************************

------------------------------------------------------
-- Alternativa 1: Usando Operador SET (UNION)
------------------------------------------------------
-- Esta consulta obtiene la fecha de emisión del informe (SYSDATE),
-- la descripción del tipo de transacción, y el promedio de montos
-- para transacciones con cuotas entre junio (06) y diciembre (12).
-- Se realizan dos consultas unidas por UNION para demostrar
-- el uso del operador SET.

SELECT 
    SYSDATE AS FECHA,
    trans.cod_tptran_tarjeta AS CODIGO,
    UPPER(tipo.nombre_tptran_tarjeta) AS DESCRIPCION,
    ROUND(AVG(cuota.valor_cuota)) AS "MONTO PROMEDIO TRANSACCION"
FROM 
    cuota_transac_tarjeta_cliente cuota,
    transaccion_tarjeta_cliente trans,
    tipo_transaccion_tarjeta tipo
WHERE 
    cuota.nro_tarjeta = trans.nro_tarjeta
    AND cuota.nro_transaccion = trans.nro_transaccion
    AND trans.cod_tptran_tarjeta = tipo.cod_tptran_tarjeta
    AND TO_NUMBER(TO_CHAR(cuota.fecha_venc_cuota, 'MM')) BETWEEN 6 AND 12
GROUP BY 
    trans.cod_tptran_tarjeta,
...
INNER JOIN transaccion_tarjeta_cliente trans 
    ON cuota.nro_tarjeta = trans.nro_tarjeta
    AND cuota.nro_transaccion = trans.nro_transaccion
LEFT JOIN tipo_transaccion_tarjeta tipo 
    ON trans.cod_tptran_tarjeta = tipo.cod_tptran_tarjeta
WHERE 
    TO_NUMBER(TO_CHAR(cuota.fecha_venc_cuota, 'MM')) BETWEEN 6 AND 12
GROUP BY 
    trans.cod_tptran_tarjeta,
    tipo.nombre_tptran_tarjeta

-- Ordenamos el resultado final por el MONTO PROMEDIO TRANSACCION ascendente
ORDER BY "MONTO PROMEDIO TRANSACCION" ASC;


------------------------------------------------------
-- Alternativa 2: Usando Subconsulta
-- Los resultados se almacenan en SELECCION_TIPO_TRANSACCION
------------------------------------------------------
-- En este caso, primero realizamos una subconsulta que filtra cuotas
-- entre junio y diciembre. Con esa subconsulta, calculamos el promedio
-- y luego insertamos en la tabla SELECCION_TIPO_TRANSACCION.
...
SELECT 
    TO_CHAR(SYSDATE,'DD-MON-YY') AS FECHA,                -- Fecha actual (hoy)
    trans.cod_tptran_tarjeta AS CODIGO,                   -- Código del tipo de transacción
    UPPER(tipo.nombre_tptran_tarjeta) AS DESCRIPCION,     -- Descripción en mayúsculas
    ROUND(AVG(cuota_final.valor_cuota)) AS PROMEDIO       -- Promedio consolida un valor por código
FROM
    (
        SELECT nro_tarjeta, nro_transaccion, fecha_venc_cuota, valor_cuota
        FROM cuota_transac_tarjeta_cliente
        WHERE TO_NUMBER(TO_CHAR(fecha_venc_cuota, 'MM')) BETWEEN 6 AND 12
    ) cuota_final
INNER JOIN transaccion_tarjeta_cliente trans
    ON cuota_final.nro_tarjeta = trans.nro_tarjeta
   AND cuota_final.nro_transaccion = trans.nro_transaccion
INNER JOIN tipo_transaccion_tarjeta tipo
    ON trans.cod_tptran_tarjeta = tipo.cod_tptran_tarjeta
GROUP BY 
    trans.cod_tptran_tarjeta,
    tipo.nombre_tptran_tarjeta
ORDER BY 
    ROUND(AVG(cuota_final.valor_cuota)) ASC;

COMMIT;

-- Ahora consultamos los datos recién insertados. 
-- Cada código debería aparecer sólo una vez, con su promedio ya calculado.
SELECT 
    fecha AS FECHA,
    cod_tipo_transac AS CODIGO,
    nombre_tipo_transac AS DESCRIPCION,
    monto_promedio AS "MONTO PROMEDIO TRANSACCION"
FROM seleccion_tipo_transaccion
ORDER BY "MONTO PROMEDIO TRANSACCION" ASC;



TRUNCATE TABLE seleccion_tipo_transaccion;

------------------------------------------------------
-- Actualización de la tasa de interés en TIPO_TRANSACCION_TARJETA
-- Rebaja del 1% según la información en SELECCION_TIPO_TRANSACCION.
------------------------------------------------------
-- Consideramos las transacciones de los últimos 6 meses. 
-- La subconsulta se basa en la fecha_informe (fecha de emisión del informe)
-- almacenada en SELECCION_TIPO_TRANSACCION, y comparamos la fecha_informe
-- con un rango entre TRUNC(ADD_MONTHS(SYSDATE, -6), 'MM') y TRUNC(SYSDATE, 'MM').

UPDATE tipo_transaccion_tarjeta tipo
SET tipo.tasaint_tptran_tarjeta = tipo.tasaint_tptran_tarjeta - 1
WHERE tipo.cod_tptran_tarjeta IN (
    SELECT DISTINCT t3.cod_tptran_tarjeta
    FROM tipo_transaccion_tarjeta t3
    INNER JOIN transaccion_tarjeta_cliente t2
        ON t3.cod_tptran_tarjeta = t2.cod_tptran_tarjeta
    INNER JOIN cuota_transac_tarjeta_cliente c2
        ON t2.nro_tarjeta = c2.nro_tarjeta 
       AND t2.nro_transaccion = c2.nro_transaccion
    WHERE TO_NUMBER(TO_CHAR(c2.fecha_venc_cuota, 'MM')) BETWEEN 6 AND 12
      AND EXISTS (
          SELECT 1 
          FROM seleccion_tipo_transaccion stt
          WHERE UPPER(t3.nombre_tptran_tarjeta) = stt.nombre_tipo_transac
            AND TO_DATE(stt.fecha, 'DD-MON-YY') BETWEEN TRUNC(ADD_MONTHS(SYSDATE, -6), 'MM') 
...
ORDER BY tasaint_tptran_tarjeta ASC;
...
1. ¿Cuál es el problema que se debe resolver?
   Se debe identificar las transacciones de tarjeta de crédito cuyas cuotas vencen entre junio y diciembre, 
   calcular el promedio de los montos por tipo de transacción, mostrar la información con la fecha del informe 
   y guardar dichos resultados (al menos en la alternativa con subconsulta) en la tabla SELECCION_TIPO_TRANSACCION. 
   Finalmente, se debe actualizar la tasa de interés de los tipos de transacción identificados, reduciéndola en 1%.

2. ¿Cuál es la información significativa que necesita para resolver el problema?
   - Cuotas y su fecha de vencimiento (tabla CUOTA_TRANSAC_TARJETA_CLIENTE).
   - El tipo de transacción asociada a cada cuota (TRANSACCION_TARJETA_CLIENTE y TIPO_TRANSACCION_TARJETA).
   - Fechas y promedios de montos para filtrar, calcular y mostrar.
   - La fecha actual (SYSDATE) para la fecha de informe y para cálculos de rangos temporales.

3. ¿Cuál es el propósito de la solución que se requiere?
   Mostrar las transacciones con cuotas vencidas entre junio y diciembre, calculando el promedio de sus montos, 
   insertarlas en SELECCION_TIPO_TRANSACCION (en la alternativa con subconsulta) y luego actualizar la tasa de interés 
   en TIPO_TRANSACCION_TARJETA aplicando una reducción del 1%.

4. Pasos, en lenguaje natural, necesarios para la alternativa que usa SUBCONSULTA:
   a. Crear una subconsulta que filtre cuotas entre junio y diciembre.
   b. Unir esa subconsulta con las tablas de transacciones y tipos de transacción.
   c. Calcular el promedio de montos, formatear las cadenas según requisitos, agregar SYSDATE como fecha de informe.
   d. Insertar los resultados en SELECCION_TIPO_TRANSACCION.
   e. Ordenar por el promedio de montos ascendente.
   f. Usar la información de SELECCION_TIPO_TRANSACCION para actualizar la tasa de interés en TIPO_TRANSACCION_TARJETA.

5. Pasos, en lenguaje natural, necesarios para la alternativa que usa OPERADOR SET:
   a. Realizar dos consultas independientes que obtengan la información de cuotas, tipos de transacción, promedio de montos y la fecha del informe (SYSDATE).
   b. Filtrar las cuotas entre junio y diciembre.
   c. Calcular el promedio de montos y transformar las columnas (UPPER, ROUND, etc.).
   d. Combinar las dos consultas usando UNION.
   e. Ordenar el resultado final por el promedio de montos ascendente.
   f. (La actualización del 1% de tasa también se puede basar en la información generada si se almacenara, aunque en la alternativa con UNION no se pide guardar los datos, únicamente mostrarlos.)
